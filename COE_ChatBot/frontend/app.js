...
async function sendMessage() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            addMessageToChat('bot', data.error || 'Sorry, something went wrong. Please try again.');
        }
        ...
        addMessageToChat('bot', '⚠️ Connection error. Please check if the backend server is running on port 5000.');
    }
}
...
function addBotResponse(data) {
    ...
    if (data.steps && data.steps.length > 0) {
        ...
        stepsList.innerHTML = '<strong>📝 Steps:</strong><ol>' + 
    ...
    if (data.important_notes && data.important_notes.length > 0) {
        ...
        notesList.innerHTML = '<strong>⚠️ Important Notes:</strong><ul>' + 
            data.important_notes.map(note => `<li>${formatText(note)}</li>`).join('') + 
            '</ul>';
    ...
    if (data.additional_info && data.additional_info.length > 0) {
        ...
        infoList.innerHTML = '<strong>ℹ️ Additional Information:</strong><ul>' + 
            data.additional_info.map(info => `<li>${formatText(info)}</li>`).join('') + 
            '</ul>';
    ...
    if (data.related_links && data.related_links.length > 0) {
        ...
        linksList.innerHTML = '<strong>🔗 Related Links:</strong><ul>' + 
    ...
    if (data.suggestions && data.suggestions.length > 0) {
        ...
        suggestionsList.innerHTML = '<strong>💡 I can help you with:</strong><ul>' + 
            data.suggestions.map(suggestion => `<li>${formatText(suggestion)}</li>`).join('') + 
            '</ul>';
